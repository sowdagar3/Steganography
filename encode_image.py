import os
import glob
import bchlib #This is a python module for encoding and correcting data using BCH codes.
import numpy as np
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default=r'./out')
    ...
    data = bytearray(args.secret + ' ' * (7 - len(args.secret)), 'utf-8')#To encode a text string into UTF-8 format, each character is first #converted into its corresponding Unicode code point, and then the code point is encoded into a sequence of bytes
    ecc = bch.encode(data)
    packet = data + ecc
